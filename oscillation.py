...
model = ResonanceNetwork(input_nnodes=len(train_data[0]), nnodes=10, p_link=0.1, leak=0.75, lrate_targ=0.01, lrate_wmat=0.1, targ_min=1)
results = model.run(train_data)

# Cue trained model

state = 0 # select matrix for correlations: 0 - spikes, 1 - activations, 2 - reservoir weights
